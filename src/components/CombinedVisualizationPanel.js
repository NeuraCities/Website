...
import InfrastructureConditionDashboard from './artifacts/1.0-chart';
...
const mapContainerRef = useRef(null);
  ...
  // Handle map saving
  const handleSaveMap = async (mapName) => {
...
// Inside CombinedVisualizationPanel component
const renderContent = () => {
  // Check if artifacts and currentArtifactId are properly passed
  if (!artifacts || !currentArtifactId) {
    // Fall back to the regular content rendering
    return <div dangerouslySetInnerHTML={mapMarkup} />;
  }
  ...
  // Determine which component to render based on artifact title
  if (currentArtifact.title === '1.0 Infrastructure Critical Zones Map') {
    return <InfrastructureConditionDashboard />;
  } else if (currentArtifact.title === '1.0 Infrastructure Condition Analysis') {
    return <InfrastructureConditionDashboard />;
  }
  
  // Default content rendering
  return <div dangerouslySetInnerHTML={{ __html: currentArtifact.content }} />;
};
  // Now prepare data for rendering after tables is initialized
  const showMapPlaceholder = !mapContent?.trim();
...
>
          ...
          <div className="flex justify-between items-center bg-white text-gray-800 border-b border-gray-200 shrink-0 px-2" 
   style={{ 
...
{resizeTooltip.visible && (
  ...
  return (
    <>
      ...
      {/* Add the save map dialog */}
      <SaveMapDialog
        isOpen={saveMapDialogOpen}
        onClose={() => setSaveMapDialogOpen(false)}
        onSave={handleSaveMap}
        defaultName={`Map ${new Date().toLocaleDateString()}`}
      />
    </>
  );
}